...
def select(sql, args, size=None):
    ...
    with(yield from __pool)as conn:
        ...
        yield from cur.execute(sql.replace('?', '%s'), args or ()) # 这是啥??? 为什么有or()
        if size:
...
class Model(dict, metaclass=ModelMetaClass):
	...
	def save(self):
		...
		rows = yield from edecute(self.__insert__, args)
		if rows != 1:
	...
	def delete(self):
		...
		if rows !=1:
			logging.warning('failed ro delete by primary key : affected rows : %s ' % rows)
