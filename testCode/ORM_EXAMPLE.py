...
# 这里有一个关于Pool的连接，讲了一些Pool的知识点，挺不错的，点击打开链接，下面这些参数都会讲到，以及destroy__pool里面的wait_closed()
...
def select(sql, args, size=None):
    ...
    # yield from从连接池中返回一个连接， 这个地方已经创建了进程池并和进程池连接了，进程池的创建被封装到了create_pool(loop,
    # **kw)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def delete(self):
        ...
        if rows != 1:
            logging.warning(
                'failed to delete by primary key: affected rows: %s' % rows)
